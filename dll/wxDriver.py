...
class WECHAT_HTTP_API_PARAM_TEMPLATES:
    __HTTP_API_PARAM_TEMPLATE = {
        ...
        APIS.WECHAT_LOGOUT: {},
        APIS.WECHAT_GET_TRANSFER: {"wxid": "",
                                   "transcationid": "", "transferid": ""}
    }
...
def test_send_msg(test_port):
    ...
    if post_wechat_http_api(APIS.WECHAT_IS_LOGIN, port=test_port)["is_login"] == 1:
        ...
        post_wechat_http_api(APIS.WECHAT_MSG_SEND_TEXT,
                             data=data, port=test_port)

        data = {"receiver": 'filehelper',
                "shared_wxid": "filehelper", "nickname": "文件传输助手"}
        post_wechat_http_api(APIS.WECHAT_MSG_SEND_CARD,
                             data=data, port=test_port)

        data = {"receiver": 'filehelper',
                "img_path": r"D:\VS2019C++\MyWeChatRobot\test\测试图片.png"}
        post_wechat_http_api(APIS.WECHAT_MSG_SEND_IMAGE,
                             data=data, port=test_port)

        data = {"receiver": 'filehelper',
                "file_path": r"D:\VS2019C++\MyWeChatRobot\test\测试文件"}
        post_wechat_http_api(APIS.WECHAT_MSG_SEND_FILE,
                             data=data, port=test_port)
        ...
        data = {"wxid": 'filehelper',
                ...
                "img_path": ""}
        post_wechat_http_api(APIS.WECHAT_MSG_SEND_ARTICLE,
                             data=data, port=test_port)
        print(post_wechat_http_api(APIS.WECHAT_CONTACT_GET_LIST, port=test_port))
        data = {"wxid": "filehelper"}
        print(post_wechat_http_api(
            APIS.WECHAT_CONTACT_CHECK_STATUS, data=data, port=test_port))
...
def test_get_chatroom_list_from_db(test_port):
    dbs = post_wechat_http_api(
        APIS.WECHAT_DATABASE_GET_HANDLES, port=test_port)
    db_handle = [i for i in dbs['data']
                 if i['db_name'] == 'MicroMsg.db'][0]['handle']
    ...
    res = post_wechat_http_api(
        APIS.WECHAT_DATABASE_QUERY, data=data, port=test_port)
    return res['data']
...
if __name__ == '__main__':
    ...
    # rs = post_wechat_http_api(APIS.WECHAT_LOG_START_HOOK, port)
    # print(post_wechat_http_api(APIS.WECHAT_GET_SELF_INFO, port))
    # post_wechat_http_api(APIS.WECHAT_MSG_SEND_TEXT, 8000, {
    #                      "wxid": "wangshi__chen", "msg": "123"})
    start_socket_server(10808, ReceiveMsgSocketServer, True)
    # while True:
    #     time.sleep(1)
    # stop_listen(pids[0])
